...
# The local client gets the same audio-follow interposer the launcher gives the real one, so
# a test client behaves like the thing being tested. FFXI_AUDIOFOLLOW_DEBUG=1 makes it talk.
AUDIOFOLLOW="${VG_AUDIOFOLLOW:-/Applications/FFXI-on-Mac.app/Contents/Resources/audiofollow.dylib}"
...
start() {
  ...
  env -i HOME="$HOME" USER="$USER" PATH=/usr/bin:/bin:/usr/sbin:/sbin SHELL=/bin/zsh \
    ...
    DYLD_INSERT_LIBRARIES="$AUDIOFOLLOW" FFXI_AUDIOFOLLOW_DEBUG="${VG_AF_DEBUG:-0}" \
    WINELOADER="${VG_WINELOADER:-$WINE}" \
